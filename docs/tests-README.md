...
* **OpenRGB streamer tests (`test_openrgb_stream.py`)**: A fake SDK server on a loopback port serves controller descriptions and records what `scripts/rgb/openrgb_stream.py` sends: the mode command once per controller at connect and never to one already in Direct (it is the command that blinks), then one UpdateLEDs frame per streaming controller per tick and none once the colour has arrived, a new target reached as a ramp, name and type exclusions never written to, and the wire format RGBController.cpp parses. Stdlib only; no OpenRGB needed.

* **Settings row grammar tests (`test_settings_row_grammar.py`)**: The row shapes Settings > Capture is the reference page for (AGENT.md's design-language section): the six widgets that carry them - `CatalogueRow`, `ConfigSwitch`, `ConfigSelectionArray`, `ConfigComboBox`, `ConfigTextArea`, `ContentSubsection` - declare each piece as an opt-in, read `Appearance` for every radius, font size and colour, write no duration or easing, and take every `Behavior`'s tier from its factory; and the page uses every piece - a plain icon on every toggle row (the chip behind it was retired on 2026-08-30; the same test refuses `iconChip` anywhere under `modules/`), an icon on every option, the quality hint computed through `record_bitrate.js` from `HyprlandData.monitors` with no placeholder screen, one recommended codec, floating labels on every field, an icon on every subsection, and rationale on the (i) rather than in a paragraph. A page adopting the grammar later is pinned here.
...
* **Curation tests (`tst_model_curation.qml`)**: the surfacing fold - empty selection surfaces everything, a named selection exactly those, copy-on-write toggles, provider index from the key_id. The skeleton contract pins the picker filter to the fold and the browse view's provider writes to the curation list alone.
* **OpenRouter mapping tests (`tst_openrouter_models.qml`)**: the /models entry mapping (provider from the id, vision from input modalities, reasoning from supported parameters, per-token prices), the filter, the import entry shape and its dedupe, and the per-million price label. **Draft tests (`tst_ai_drafts.qml`)**: the drafts fold - empty text deletes, pruning follows deleted sessions, the new-chat slot survives. The skeleton contract pins the browse view to the one switcher, its import to extraModels alone, and drafts away from the sessions index.
...
* **Ripple tone lint (`lint_ripple_tone.py`)**: a transparent-plated `RippleButton` must name its ripple tone (or disable the ripple) - the inherited default, `colLayer1Active`, is chosen for a button plated in `colLayer1`, and a transparent plate usually sits ON that very surface, so the default ripples in the colour of its own floor and reads as nothing. 33 call sites had it before the 2026-08-31 sweep; the lint keeps the class at zero.
* **Combo box press tests (`test_combo_box_press.py`)**: both combo boxes answer a press the way every other control does - a `PassiveRippleSurface` (a `RippleButton` whose containment mask holds no area) sits under each as its background, painting the hover, the ripple and the lift while never seeing the press, so the `ComboBox` still opens its popup on its own release and a delegate still reports the click that chooses a row.
* **Keyring on-demand tests (`test_keyring_on_demand.py`)**: the keyring loads on demand, so every demand has to ask. The custom-provider path did not: with a local model selected nothing loaded the keyring, the settings key fields read "" and dropped what was typed into them, and every "Fetch Models" went out with an empty bearer - a 401 the page called "Failed to fetch", whatever key the user had entered.
* **Selection array flow tests (`test_selection_array_flow.py`)**: a segmented row's chips lay out on their summed natural width whether or not the row has a label. The preferred-width escape from the Flow/implicitWidth circle (4ef84e521) applied only to labelled rows, and the Quick page's label-less rows latched one chip per line at a narrow intermediate width.
* **SNI watchdog tests (`test_sni_watchdog.py`)**: the tray watchdog's planner decides one poll tick as a pure function, without a bus - xapp kept present, an absent watcher reactivated, remembered standard-path items re-registered after a flap. The daemon half (`scripts/tray/sni_watchdog.py`) is flock-guarded and launched from the shell.
* **Subject-mask lasso tests (`test_subject_mask_lasso.py`)**: the hand-drawn half of `select` - the `--lasso LABEL:x,y;x,y;...` grammar, the rasterised arithmetic (label 1 unions, label 0 subtracts, in draw order, so a cut after an add wins where they overlap), the prompt round-trip that keeps loops and clicks in one recorded gesture, and the lasso-only path proven by running the full select() against a real temp image with no ONNX session anywhere near it.
* **Media art-trim tests (`test_media_art_trim.py`)**: the letterbox probe (`scripts/media/art_trim.py`) on synthetic Pillow images - a letterboxed square found within tolerance, a full-bleed cover left alone, one moody dark edge read as composition (no symmetric pair, no trim), slivers below the 2% floor read as noise, and the CLI answering JSON on success and failure alike.
* **Lyrics provider tests (`test_lyrics_providers.py`)**: the fetch chain in `scripts/lyrics/lyrics.py` against stubbed HTTP - the lrc and ttml parsers (word stamps included), the search validation that killed the Darkside-for-Faded mismatch (artist must match, a known duration must agree within five seconds), the provider order (GlassyMusic's own BetterLyrics DOM over CDP answers first - `glassy_dom.py`, track-guarded and failing soft - then BetterLyrics' community API with plain-format rows refused, then lrclib with exact get beating search), and the DOM payload's pure transform with its wrong-track refusals.
...
* **Installer cache housekeeping tests (`sdata/tests/test_we_cache_prune.py`, `sdata/tests/test_install_housekeeping.py`)**: Source the prune helpers out of `4.wallpaperengine.sh` and `sdata/lib/functions.sh` and run them against scratch trees: only the pinned prebuilt survives, the source checkout goes once a prebuilt is installed, QML-cache entries and crash dumps untouched for 30 days are dropped while fresh ones and anything outside those two directories stay, and the TUI rotates its install logs. `run_tests.sh` now runs every `sdata/tests/test_*.py` whenever the checkout is beside it (a deployed copy of the suite skips them and says so) - before this they were never invoked by the suite or CI.
...
* **Dumb-widget lint (`lint_dumb_widgets.py`)**: A shared widget is presentational. Every file in `modules/common/widgets` and every component the Components gallery catalogues is scanned, comments stripped, for a service singleton (anything under `services/` except `Translation`), a `Config.options` read, a `GlobalStates` read or write, or a `Process`/`execDetached` - and fails on any of them, with the fix spelled out: move the file to its consumer, or turn the call into a signal and the read into a property the host answers. Two exclusions remain, each with its reason (the notification seam, the popup protocol), and two further checks keep that list honest: an entry that no longer offends must be deleted, and an entry naming a file that has left the folder must be deleted. Mutation-checked against a `GlobalStates` read planted in a catalogued row.
* **Component gallery lint (`lint_component_gallery.py`)**: Every `RippleButton` descendant in the shell is either catalogued in `CheatsheetComponents.qml` or excluded with a reason, every catalogued path exists, and every entry's `toggles:` claim matches its type - a toggle HAS the property (a `RippleButton` descendant, or a type declaring its own) AND reads it; inheriting alone put a dead switch on 46 pages, the word alone put a broken one on two composites. Entries are read as balanced `{...}` objects and the array's shape is checked between them, because the first version of the claim lint was satisfied by a `toggles: true` that had landed outside its entry - a syntax error that took the whole tab down while the lint stayed green.
* **Bar popup slot tests (`tst_bar_popup_slot.qml`)**: The rules for which of the bar's ten popups gets its one shared card, as a pure function in `modules/common/functions/bar_popup_slot.js`: an empty slot is granted, a hover takes the card from an unpinned neighbour, a pinned occupant holds it against a hover, a second deliberate click outranks the first, Edit Mode refuses every claim including the one already holding, and holding already is not a swap. Plus the other half of a handover - when the outgoing popup drops its hover grace. Lives in a `.js` because the QML suite substitutes a double for `GlobalStates`, and a rule written there would be tested through a copy of itself.
