...
Singleton {
    ...
    function loadFromConfig() {
        ...
        if (!cfg) return
        // Expressive stores API keys in the keyring (set via launcher "/wallhaven <key>").
        // Fall back to a Config key so a manually-entered value still works.
    ...
    function saveToConfig() {
        const cfg = Config.options?.wallpaperSelector
        if (!cfg) return
        // Persist non-sensitive filters in Config, and mirror the API key into the keyring
        // so the launcher "/wallhaven <key>" command stays authoritative.
