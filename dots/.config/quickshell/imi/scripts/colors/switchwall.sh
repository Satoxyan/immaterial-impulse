...
get_video_fps() {
    local f="$1"
    local r=$(ffprobe -v 0 -of csv=p=0 -select_streams v:0 -show_entries stream=r_frame_rate -i "$f" 2>/dev/null | head -1)
    # r like 60000/1001 -> eval
    if [[ "$r" == *"/"* ]]; then
        local num=${r%/*} den=${r#*/}
        if [ "$den" != "0" ] && [ -n "$num" ]; then echo "scale=2; $num/$den" | bc 2>/dev/null | xargs printf "%.2f" 2>/dev/null || echo "$r"; else echo ""; fi
    else echo "$r"; fi
}
...
create_restore_script() {
    local video_path=$1
    local opts=$(build_video_opts "$video_path")
    local prefix=$(get_gpu_prefix)
    cat > "$RESTORE_SCRIPT.tmp" << EOF
#!/bin/bash
# Generated by switchwall.sh - Don't modify it by yourself.
# Time: $(date)
# GPU prefix: $prefix | opts: $opts | fps native max

pkill -f -9 '[m]pvpaper'

for monitor in \$(hyprctl monitors -j | jq -r '.[] | .name'); do
    $prefix mpvpaper -o "$opts" "\$monitor" "$video_path" &
    sleep 0.1
done
EOF
...
>>>>>>> theirs
                ...
                for monitor in $monitors; do
                    # ponytail: dedicated GPU first via prefix, fps native max via opts
                    if [ -n "$prefix" ]; then
                        # shellcheck disable=SC2086
                        eval "$prefix mpvpaper -o \"$opts\" \"\$monitor\" \"$video_path\" &"
                    else
                        mpvpaper -o "$opts" "$monitor" "$video_path" &
                    fi
                    sleep 0.1
                done
            fi
